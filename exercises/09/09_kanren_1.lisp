...
;; Nejjednodušším termem je konstanta.
;; Jako konstanty budeme používat čísla, řetězce a symboly.
;;
...
(defun extend-substitution (var term substitution)
  (apply #'sub (cons var term) (cdr substitution)))
...
;; Cíl určuje jak z jistého výchozí stavu určit strom stavů, 
;; který cíl splňují.

;; Cíl je funkce, která očekává stav a vrací strom stavů.
;;
;; Například: (lambda (state) (make-tree state nil)) nebo (lambda (state) (make-tree nil nil))

;; Strom stavů je prázdný, pokud neobsahuje žádný stav.
;;
;; Například strom stavů (tree nil (tree nil) (tree nil)) je prázdný.
;;
;; Cíl je pro zadaný výchozí stav splněn, pokud je jeho strom stavů neprázdný.
;;
;; Například cíl (lambda (state) (tree-node state nil)) je splněn pro každý stav,
;; ale cíl (lambda (state) (tree-node nil nil)) není splněn pro žádný stav.
...
;; Logický predikát je funkce, která pro termy vrací cíl.

;; Logický predikát == vrací pro dva termy cíl, který je splněn, pokud
;; je lze v zadaném stavu unifikovat. 
(defun == (value1 value2)
  (lambda (state)
...
#|
(apply-goal (== ?0 'a) *empty-state*)
(apply-goal (== ?0 'b) (state (sub '(?0 . a)) 1))
|#

;; Logická formule je výraz, který se vyhodnotí na cíl.
;; Například (== ?0 'a) je formule.
...
;; Pokus o splnění cíle probíhá tak, že
;; se zavolá funkce function a na vstup dostane čerstvou proměnnou.
;; Funkce function musí vrátit cíl o jehož dosažení se budeme dále pokoušet.
...
;; Logická spojka je funkce, která očekává a vrací cíle.

;; Logická spojka nebo pro dva cíle vrací cíl, který je splněn
;; pokud splněn aspoň jeden z cílů.
...
#|
(apply-goal (disj2 (== ?1 'a) (== ?1 'b)) *empty-state*)
|#


;; Získání stromu stavů splňující cíl pro libovolný stav stromu.
...
;; Logická spojka a současně pro dva cíle vrací cíl, který je splněn
;; pokud jsou splněny oba cíle.
...
#|
(apply-goal (conj2 (is ?1 'a) (is ?2 'b)) *empty-state*)
|#
...
#|
(state-tree-values-dfs (tree-node nil (list (tree-node nil (list (tree-node nil nil) (tree-node (state (sub '(?0 . 3)) 1) nil)))
                                            (tree-node (state (sub '(?0 . 1)) 0) nil) (tree-node (state (sub '(?0 . 2)) 1) nil))))
|#
...
#|
(state-tree-values-bfs (tree-node nil (list (tree-node nil (list (tree-node nil nil) (tree-node (state (sub '(?0 . 3)) 1) nil)))
                                            (tree-node (state (sub '(?0 . 1)) 0) nil) (tree-node (state (sub '(?0 . 2)) 1) nil))))
